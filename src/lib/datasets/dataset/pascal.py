...
class PascalVOC(data.Dataset):
  ...
  def __init__(self, opt, split):
    ...
    self.cocoCat_to_vocCat = {v:i+1 for i,v in enumerate(self._valid_ids_voc)}  #根据通道位置转换为coco类别id后，在根据这个字典转换为pscal voc id
    #coco中按0-79编号时，novel_class按顺序在在其中的索引，也就是pscal voc各类在通道中的位置
  ...
  def convert_eval_format(self, all_bboxes):
    ...
    for image_id in all_bboxes:  #all_boxes是字典，key是图片id，value是results
      for cls_ind in all_bboxes[image_id]:   #cls_ind : 1~80
        category_id = self.cocoCat_to_vocCat.get(self._valid_ids[cls_ind - 1],0)  # 如cls_ind - 1对应的coco类id不在voc id中，则返回0，记为背景；反之，记录其在Pascal voc下的类别号（1-20）
  ...
  def run_eval(self, results, save_dir,test_directly=False):
    # result_json = os.path.join(save_dir, "results.json")
    # detections  = self.convert_eval_format(results)
    # json.dump(detections, open(result_json, "w"))
